...
var localMessagesStore = [];
var localConversationsStore = [];
async function dbGetMessages(conversationId) {
  const database = await getDb();
  if (database) {
    try {
      const col = database.collection("chat_messages");
      return await col.find({ conversationId }).sort({ timestamp: 1 }).toArray();
    } catch (err) {
      console.error("MongoDB get messages error:", err);
    }
  }
  return localMessagesStore.filter((m) => m.conversationId === conversationId).sort((a, b) => a.timestamp.localeCompare(b.timestamp));
}
async function dbSaveChatMessage(message) {
  const database = await getDb();
  if (database) {
    try {
      const col = database.collection("chat_messages");
  ...
  localMessagesStore.push(message);
  await dbUpdateConversationSummary(message);
  return message;
}
async function dbGetConversations() {
  ...
  if (database) {
    try {
      ...
      return await col.find({}).sort({ lastMessageAt: -1 }).toArray();
    } catch (err) {
      console.error("MongoDB get conversations error:", err);
    }
  }
  return [...localConversationsStore].sort((a, b) => b.lastMessageAt.localeCompare(a.lastMessageAt));
}
async function dbMarkMessagesAsRead(conversationId, senderIdToMarkRead) {
  const database = await getDb();
  if (database) {
    try {
      const col = database.collection("chat_messages");
      await col.updateMany(
        { conversationId, senderId: senderIdToMarkRead, isRead: false },
        { $set: { isRead: true } }
      );
      const convCol = database.collection("conversations");
      ...
      console.error("MongoDB mark messages as read error:", err);
    }
  }
  localMessagesStore.forEach((m) => {
    if (m.conversationId === conversationId && m.senderId === senderIdToMarkRead) {
      m.isRead = true;
    }
  });
  const conv = localConversationsStore.find((c) => c.id === conversationId);
  if (conv) {
    conv.unreadCount = 0;
  }
  return true;
}
async function dbDeleteConversation(conversationId) {
  const database = await getDb();
  if (database) {
    try {
      const col = database.collection("conversations");
      await col.deleteOne({ id: conversationId });
      const msgCol = database.collection("chat_messages");
      await msgCol.deleteMany({ conversationId });
      return true;
    } catch (err) {
      console.error("MongoDB delete conversation error:", err);
    }
  }
  const convIndex = localConversationsStore.findIndex((c) => c.id === conversationId);
  if (convIndex !== -1) {
    localConversationsStore.splice(convIndex, 1);
  }
  for (let i = localMessagesStore.length - 1; i >= 0; i--) {
    if (localMessagesStore[i].conversationId === conversationId) {
      localMessagesStore.splice(i, 1);
    }
  }
  return true;
}
async function dbUpdateConversationSummary(message) {
  const conversationId = message.conversationId;
  const isFromAdmin = message.senderId === "admin";
  const userId = conversationId.replace("conv_", "");
  let userDetails = { name: message.senderName, email: "" };
  try {
    const user = await dbFindUserByEmail(message.senderName);
    if (user) {
      userDetails.name = user.name;
      userDetails.email = user.email;
    } else {
      const allUsers = await dbGetAllUsers();
      const matchedUser = allUsers.find((u) => u.id === userId || u.email === userId);
      if (matchedUser) {
        userDetails.name = matchedUser.name;
        userDetails.email = matchedUser.email;
      }
    }
  } catch (err) {
    console.error("Error looking up user details for conversation summary:", err);
  }
  const database = await getDb();
  if (database) {
    try {
      const col = database.collection("conversations");
      const existingConv = await col.findOne({ id: conversationId });
      let newUnreadCount2 = 0;
      if (existingConv) {
        newUnreadCount2 = isFromAdmin ? 0 : (existingConv.unreadCount || 0) + 1;
      } else {
        newUnreadCount2 = isFromAdmin ? 0 : 1;
      }
      const updatedConv = {
        id: conversationId,
        userId,
      ...
      console.error("MongoDB update conversation summary error:", err);
    }
  }
  const existingLocalIndex = localConversationsStore.findIndex((c) => c.id === conversationId);
  let newUnreadCount = isFromAdmin ? 0 : 1;
  if (existingLocalIndex !== -1) {
    newUnreadCount = isFromAdmin ? 0 : (localConversationsStore[existingLocalIndex].unreadCount || 0) + 1;
  }
  ...
  if (existingLocalIndex !== -1) {
    localConversationsStore[existingLocalIndex] = updatedLocalConv;
  } else {
    localConversationsStore.push(updatedLocalConv);
  }
}
var app = express();
...
app.post("/api/chat/message", async (req, res) => {
  try {
    ...
    const saved = await dbSaveChatMessage(newMessage);
...
app.post("/api/chat/read", async (req, res) => {
  try {
    const { conversationId, senderIdToMarkRead } = req.body;
    if (!conversationId || !senderIdToMarkRead) {
      return res.status(400).json({ error: "Conversation ID and sender ID to mark read are required." });
    }
    await dbMarkMessagesAsRead(conversationId, senderIdToMarkRead);
...
var configPath = path.join(process.cwd(), "payment_config.json");
var getPaymentConfig = () => {
  try {
    if (fs.existsSync(configPath)) {
      const parsed = JSON.parse(fs.readFileSync(configPath, "utf8"));
      if (!parsed.masterUnlockKey) {
        parsed.masterUnlockKey = "APEX-SOURCE-CODE-2026";
      }
      return parsed;
    }
  } catch (e) {
  ...
  return { customPaymentLink: "", masterUnlockKey: "APEX-SOURCE-CODE-2026" };
};
var savePaymentConfig = (config) => {
  try {
    fs.writeFileSync(configPath, JSON.stringify(config, null, 2), "utf8");
  } catch (e) {
    console.warn("Failed to write payment config:", e);
  }
};
app.get("/api/payment-config", (req, res) => {
  const config = getPaymentConfig();
  const hasUploadedZip = fs.existsSync("/tmp/uploaded-source.zip");
  res.json({
...
app.get("/api/admin/payment-config", (req, res) => {
  ...
  res.json({
    customPaymentLink: config.customPaymentLink || "",
    masterUnlockKey: config.masterUnlockKey || "APEX-SOURCE-CODE-2026",
    hasUploadedZip
  });
});
app.post("/api/admin/payment-config", (req, res) => {
  const { customPaymentLink, masterUnlockKey } = req.body;
  const config = getPaymentConfig();
  config.customPaymentLink = customPaymentLink || "";
  config.masterUnlockKey = masterUnlockKey || "APEX-SOURCE-CODE-2026";
  savePaymentConfig(config);
  res.json({
    success: true,
    customPaymentLink: config.customPaymentLink,
    masterUnlockKey: config.masterUnlockKey
  });
});
app.post("/api/verify-license", (req, res) => {
  ...
  const config = getPaymentConfig();
  const correctKey = (config.masterUnlockKey || "APEX-SOURCE-CODE-2026").trim().toLowerCase();
  if (licenseKey.trim().toLowerCase() === correctKey) {
    return res.json({ success: true });
  }
  return res.status(400).json({ error: "Invalid license key. Please check your purchase confirmation." });
});
app.post("/api/payment-config", (req, res) => {
  const { customPaymentLink } = req.body;
  const config = getPaymentConfig();
  config.customPaymentLink = customPaymentLink || "";
  savePaymentConfig(config);
  res.json({ success: true, customPaymentLink: config.customPaymentLink });
});
app.post("/api/admin/upload-source", express.json({ limit: "100mb" }), (req, res) => {
  try {
    ...
    const filepath = "/tmp/uploaded-source.zip";
    const dir = path.dirname(filepath);
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, { recursive: true });
    }
    fs.writeFileSync(filepath, buffer);
    console.log(`Admin uploaded source code ZIP: ${filename} (${buffer.length} bytes)`);
    res.json({ success: true, filename, size: buffer.length });
  } catch (err) {
    console.error("Failed to upload source file:", err);
    res.status(500).json({ error: err.message || "Failed to write source ZIP file." });
  }
});
app.get("/api/download-source", (req, res) => {
  ...
  if (config.customPaymentLink && config.customPaymentLink.trim()) {
    ...
    const correctKey = (config.masterUnlockKey || "APEX-SOURCE-CODE-2026").trim().toLowerCase();
    const isAdminBypass = req.query.bypass === "admin";
    if (!isAdminBypass && (!userKey || userKey.trim().toLowerCase() !== correctKey)) {
      return res.status(403).send("<h1>Access Denied</h1><p>A valid commercial license key is required to download this codebase. Please complete your payment first.</p>");
    }
  }
  const uploadedPath = "/tmp/uploaded-source.zip";
  if (fs.existsSync(uploadedPath)) {
    console.log("Serving admin-uploaded source ZIP.");
    return res.download(uploadedPath, "lms-full-source-code.zip");
  }
  if (fs.existsSync("/tmp/source.zip")) {
    try {
      fs.unlinkSync("/tmp/source.zip");
    } catch (e) {
    }
  }
  if (fs.existsSync("/tmp/source.tar.gz")) {
    try {
      fs.unlinkSync("/tmp/source.tar.gz");
    } catch (e) {
    }
  }
  const zipCommand = 'zip -r /tmp/source.zip . -x "node_modules/*" -x "dist/*" -x ".git/*" -x "*.zip" -x "*.tar.gz" -x ".npm/*" -x ".cache/*" -x ".local/*"';
  exec(zipCommand, (error) => {
    if (error) {
      console.warn("Native zip command failed, falling back to standard tar.gz packaging...", error);
      const tarCommand = 'tar -czf /tmp/source.tar.gz . --exclude="node_modules" --exclude="dist" --exclude=".git" --exclude="*.zip" --exclude="*.tar.gz" --exclude=".npm" --exclude=".cache" --exclude=".local"';
      exec(tarCommand, (tarError) => {
        if (tarError) {
          console.error("Tar fallback also failed:", tarError);
          return res.status(500).json({ error: "Failed to package and compress source code." });
        }
        res.download("/tmp/source.tar.gz", "lms-full-source-code.tar.gz");
      });
      return;
    }
    res.download("/tmp/source.zip", "lms-full-source-code.zip");
  });
});
